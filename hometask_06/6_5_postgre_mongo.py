...
class Scrapper:
    ...
    def __init__(self, page_from, page_to, limit=2):
        ...
        self.results = {}
        self.links = []
        #prepare connection to postgreSQL
        #self.__prepare_connection()
        #prepare connection to mongoDB
        self.__prepare_mongo_connection()
    ...
    def __prepare_mongo_connection(self):
        mongo_connection = pymongo.MongoClient()
        self.db = mongo_connection["test_database"]
        self.mongo_collection = self.db['hometask']
    ...
    def save_to_mongo(self):
        for v in self.results.values():
            self.mongo_collection.insert_one(v)             


    def display_data_mongo(self):
        cursor = self.db.hometask.find() 
...
if __name__ == "__main__":
    ...
    #scrapper.display_data_postgre()
    #scrapper.conn_close()
    scrapper.save_to_mongo()
    print("--- %s seconds ---" % (time.time() - start_time))
    ## 93 seconds - postgre
    ## 133 seconds - mongo 
    scrapper.display_data_mongo()
